...
//import org.geotools.data.DefaultFeatureCollection;
//import org.geotools.data.simple.SimpleFeatureBuilder;
//import org.geotools.data.simple.SimpleFeatureTypeBuilder;
...
import static edu.du.ogc.ais.examples.VerticalProfiling.pathPositions;
...
public class TrackDensityViewer {
    ...
    public void CreateDensityMap(String fileName)
    {
        //the bound should from GML
       //xmin,xmax,ymin,ymax??
     ...
     private SimpleFeatureCollection createPoints(ReferencedEnvelope bounds)
    {
        ...
        for (Position p : pathPositions) {
            Geometry point = factory.createPoint(new Coordinate (p.getLatitude().degrees, p.getLongitude().degrees,p.elevation));
            fb.add(point);
            fb.add(p.elevation);
