...
public class BasicTiledImageLayer extends TiledImageLayer implements BulkRetrievable
{
    ...
    //X-PATCH Marjan
    //added support for transparency filter
    protected ITransparencyFilter transparencyFilter = null;
    //X-END
    ...
    protected TextureData readTexture(java.net.URL url, String textureFormat, boolean useMipMaps)
    {
        try
        {
            ...
            if ("image/dds".equalsIgnoreCase(textureFormat) && !url.toString().toLowerCase().endsWith("dds"))
            {
                ...
                //X-PATCH Marjan
                //applying the layer's transparency filter to DXT comp. attributes
                attributes.setTransparencyFilter(getTransparencyFilter());
                //X-END
    ...
    {
        //X-PATCH Marjan
        //passing transp. filter to DownloadPostProcessor
        return new DownloadPostProcessor(tile, this, transparencyFilter);
        //X-ORIG
    ...
    protected static class DownloadPostProcessor extends AbstractRetrievalPostProcessor
    {
        ...
        //X-PATCH Marjan
        //added transp. filter field
        protected ITransparencyFilter transparencyFilter = null;
        //X-END

        //X-PATCH Marjan
        //added transparency filter param
        public DownloadPostProcessor(TextureTile tile, BasicTiledImageLayer layer, ITransparencyFilter transparencyFilter)
        {
            this(tile, layer, (FileStore)null);
        ...
        //X-PATCH Marjan
        //overriding convertToDDS() so that transp. filter is applied
        @Override
        protected ByteBuffer convertToDDS() throws IOException
        {
            ...
            if (image != null)
                buffer = DDSCompressor.compressImage(image, transparencyFilter);
            else {
                DXTCompressionAttributes attributes = DDSCompressor.getDefaultCompressionAttributes();
                attributes.setTransparencyFilter(transparencyFilter);
                buffer = DDSCompressor.compressImageBuffer(this.getRetriever().getBuffer(), attributes);
            }
    ...
    //X-PATCH Marjan
    //transparency filter setter/getter
    public ITransparencyFilter getTransparencyFilter()
    {
        return transparencyFilter;
    }

    public void setTransparencyFilter(ITransparencyFilter transparencyFilter)
    {
        this.transparencyFilter = transparencyFilter;
    }
    //X-END
