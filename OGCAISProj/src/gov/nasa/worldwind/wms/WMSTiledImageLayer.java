...
public class WMSTiledImageLayer extends BasicTiledImageLayer
{
    ...
    //added by jing li to handel time enabled wms
    private static volatile String timeDimesion=null;
    ...
    public WMSTiledImageLayer(WMSCapabilities caps, AVList params, String timeSeries)
    {
    	this(wmsGetParamsFromCapsDoc(caps, params));
    	setChangeTime(timeSeries);
        this.setUseTransparentTextures(true);
      
    }
    
    public static void setChangeTime(String time)
    {
    	//this method does not change the name of cache, so it wont provide new cache name for a file, only cachename is null, it can provide new file
    	timeDimesion = time;
    ...
    public static AVList wmsGetParamsFromCapsDoc(WMSCapabilities caps, AVList params)
    {
        ...
        {
        	//changed by jing
        	if (timeDimesion==null)
        	{
            DataConfigurationUtils.getWMSLayerConfigParams(caps, formatOrderPreference, params, null);
        	}
        	else
        	{
        		 DataConfigurationUtils.getWMSLayerConfigParams(caps, formatOrderPreference, params, timeDimesion);
        	}
