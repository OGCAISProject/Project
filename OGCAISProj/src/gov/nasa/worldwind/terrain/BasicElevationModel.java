...
public class BasicElevationModel extends AbstractElevationModel implements BulkRetrievable
{
	...
	@Override
	public double getElevationsImmediate(Sector sector, List<? extends LatLon> latlons, double targetResolution, double[] buffer){
        return this.getElevationsImmediate(sector, latlons, targetResolution, buffer, false);
    }

	protected double getElevationsImmediate(Sector sector, List<? extends LatLon> latlons, double targetResolution,
        double[] buffer, boolean mapMissingData)
    {
        if (sector == null)
        {
        ...
        sector = sector.intersection(this.levels.getSector());
            ...
            for (int col = nwCol; col <= seCol; col++) {
                TileKey key = new TileKey(targetLevel.getLevelNumber(), row, col, targetLevel.getCacheName());
                    try {
                        ...
                        URL url = getDataFileStore().findFile(tile.getPath(), false);
                        ...
                        if (url == null) {
                            ...
                            java.net.URL urlRemote = null;
                            try {
                                urlRemote = tile.getResourceURL();
                                if (WorldWind.getNetworkStatus().isHostUnavailable(urlRemote)) {
                                ...
                                Logging.logger().log(java.util.logging.Level.SEVERE,
                                	Logging.getMessage("TiledElevationModel.ExceptionCreatingElevationsUrl", urlRemote), e);
								return Double.MAX_VALUE;
                            }

                            DownloadPostProcessor postProcessor = new DownloadPostProcessor(tile, this);
                            
                            URLRetriever retriever = new HTTPRetriever(urlRemote, postProcessor);
                            try {
                                retriever.call();
                            } catch (Exception ex) {
                        ...
                        url = getDataFileStore().findFile(tile.getPath(), false);
                        //load from file cache
                        if (url != null) {
                            
                            boolean tileCorrupted = false;
	...
	public void forceLoadTile(ElevationTile tile, TileKey tileKey){
		try
			{
				tile = createTile(tileKey);
	...
	@Override
	public double getExactElevation(Angle latitude, Angle longitude){
	...
	@Override
        public double getExactElevation(Angle latitude, Angle longitude, double targetResolution){
...
//            Level lastLevel = this.levels.getLastLevel(latitude, longitude);
            ...
            Level targetLevel = this.getTargetLevel(new Sector(latitude, latitude, longitude, longitude), targetResolution);

            //if we have a elevation mapping in memory at a higher than target resolution, return that
            int unmappedLevelNum = getUnmappedElevationLevel(latitude, longitude);
            if (unmappedLevelNum > targetLevel.getLevelNumber())
                return getElevation(latitude, longitude);

            tileKey = new TileKey(latitude, longitude, this.levels, targetLevel.getLevelNumber());
            ElevationTile tile = this.getTileFromMemory(tileKey);
	...
	/**
	 * Retrieve current detail level for specified location. Used for correcting "exact" elevations if user zooms in on area.
	 * @param latitude  Latitude of specified location.
	 * @param longitude Longitude of specified location.
	 * @return  Level for specified location.
	 */
	public int getUnmappedElevationLevel(Angle latitude, Angle longitude)
	{
		if (latitude == null || longitude == null)
		{
			String msg = Logging.getMessage("nullValue.AngleIsNull");
