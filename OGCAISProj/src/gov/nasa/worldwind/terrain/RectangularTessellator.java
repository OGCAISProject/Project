...
/**
 * @author tag
 * @version $Id: RectangularTessellator.java 1171 2013-02-11 21:45:02Z dcollins $
 */
public class RectangularTessellator extends WWObjectImpl implements Tessellator
{
    protected static class RenderInfo
    {
		...
		private final IntBuffer indicesAdjacency;
		//X-END
        ...
        protected RenderInfo(DrawContext dc, int density, FloatBuffer vertices, Vec4 refCenter)
        {
			...
			//Vito
			//added support for GL_TRIANGLES_ADJACECY
			createIndicesAdjacency(density);
			this.indicesAdjacency = indexListsAdjacency.get(this.density);
			//X-END
    ...
    protected static class RectTile implements SectorGeometry
    {
        ...
        public void beginRendering(DrawContext dc, int numTextureUnits)
        {
            ...
            if (dc.getGLRuntimeCapabilities().isUseVertexBufferObject())
            {
				...
				//Vito
				//added support for GL_TRIANGLES_ADJACECY
                if (this.tessellator.bindVbos(dc, this, numTextureUnits, false)){
        ...
        public void renderMultiTexture(DrawContext dc, int numTextureUnits)
        {
			...
			//Vito
			//added support for GL_TRIANGLES_ADJACECY
			this.tessellator.render(dc, this, numTextureUnits, false);
			//old:
        ...
        public void renderMultiTexture(DrawContext dc, int numTextureUnits, boolean beginRenderingCalled)
        {
            if (beginRenderingCalled)
            {
                ...
                this.tessellator.render(dc, this, numTextureUnits, false);
				//old:
                //this.tessellator.render(dc, numTextureUnits, this);
				//X-END
            }
            else
            {
				...
				//Vito
				//added support for GL_TRIANGLES_ADJACECY
                this.tessellator.render(dc, this, false);
				//old:
                //this.tessellator.render(dc, numTextureUnits, this);
				//X-END
		...
		@Override
        public void beginRenderingAdjacency(DrawContext dc, int numTextureUnits)
        {
            dc.getView().setReferenceCenter(dc, ri.referenceCenter);
            if (dc.getGLRuntimeCapabilities().isUseVertexBufferObject())
            {
                if (this.tessellator.bindVbos(dc, this, numTextureUnits, true)){
                    this.ri.isVboBound = true;
				}
            }
        }
		...
		@Override
		public void renderAdjacency(DrawContext dc, int numTextureUnits, boolean beginRenderingCalled)
		{
			if(beginRenderingCalled){
            	this.tessellator.render(dc, this, numTextureUnits, true);
			}
			else{
				this.beginRendering(dc, numTextureUnits);
            	this.tessellator.render(dc, this, numTextureUnits, true);
		...
		@Override
		public void renderAdjacency(DrawContext dc, boolean beginRenderingCalled)
		{
			if(beginRenderingCalled){
            	this.tessellator.render(dc, this, true);
			}
			else{
				this.beginRendering(dc, 1);
            	this.tessellator.render(dc, this, true);
	...
	//Vito
	//added support for GL_TRIANGLES_ADJACECY
    protected static final HashMap<Integer, IntBuffer> indexListsAdjacency = new HashMap<Integer, IntBuffer>();
    protected static final HashMap<Integer, Object> indexListsVboAdjacencyCacheKeys = new HashMap<Integer, Object>();
	//X-END
	...
	//Vito
	//added support for GL_TRIANGLES_ADJACECY
    protected void renderMultiTexture(DrawContext dc, RectTile tile, int numTextureUnits, boolean renderAdjacent)
	//old:
	...
	//X-END
    {
		...
		//Vito
		//added support for GL_TRIANGLES_ADJACECY
        this.render(dc, tile, numTextureUnits, renderAdjacent);
		//old:
	...
	//Vito
	//added support for GL_TRIANGLES_ADJACECY
    protected void render(DrawContext dc, RectTile tile, boolean renderAdjacent)
	//old:
    //protected void renderMultiTexture(DrawContext dc, RectTile tile, int numTextureUnits)
	//X-END
    {
		...
		//Vito
		//added support for GL_TRIANGLES_ADJACECY
        this.render(dc, tile, 1, renderAdjacent);
		//old:
	...
	//Vito
	//added support for GL_TRIANGLES_ADJACECY
    protected long render(DrawContext dc, RectTile tile, int numTextureUnits, boolean renderAdjacent)
	//old:
	...
	//X-END
    {
        ...
        if (dc.getGLRuntimeCapabilities().isUseVertexBufferObject())
        {
			...
			//Vito
			//added support for GL_TRIANGLES_ADJACECY
            if (!this.renderVBO(dc, tile, numTextureUnits, renderAdjacent))
			//old:
			...
			//X-END
            {
				...
				//Vito
				//added support for GL_TRIANGLES_ADJACECY
                this.renderVA(dc, tile, numTextureUnits, renderAdjacent);
				//old:
	...
	//Vito
	//added support for GL_TRIANGLES_ADJACECY
    protected void renderVA(DrawContext dc, RectTile tile, int numTextureUnits, boolean renderAdjacent)
    //old:
	...
	//Vito
	//added support for GL_TRIANGLES_ADJACECY
    protected boolean renderVBO(DrawContext dc, RectTile tile, int numTextureUnits, boolean renderAdjacent)
	{
        if (tile.ri.isVboBound || this.bindVbos(dc, tile, numTextureUnits, renderAdjacent))
		{
	//old:
	...
	//Vito
	//added support for GL_TRIANGLES_ADJACECY
    protected boolean bindVbos(DrawContext dc, RectTile tile, int numTextureUnits, boolean renderAdjacent)
	//old:
	...
	//X-END
    {
		...
		//Vito
		//added support for GL_TRIANGLES_ADJACECY
        Object indexListVboCacheKey = renderAdjacent ? indexListsVboAdjacencyCacheKeys.get(tile.density) 
													 : indexListsVboCacheKeys.get(tile.density);
		//old:
        ...
        if (indexListVboId == null)
			...
			//Vito
			//added support for GL_TRIANGLES_ADJACECY
            indexListVboId = this.fillIndexListVbo(dc, tile.density, tile.ri.indices, renderAdjacent);
			//old:
	...
	//Vito
	//added support for GL_TRIANGLES_ADJACECY
    protected int[] fillIndexListVbo(DrawContext dc, int density, IntBuffer indices, boolean fillAdjacent)
	//old:
	...
	//X-END
    {
		...
		//Vito
		//added support for GL_TRIANGLES_ADJACECY
        Object indexListVboCacheKey;
		if(fillAdjacent){
        	indexListVboCacheKey = indexListsVboAdjacencyCacheKeys.get(density);
		}
		else{
        	indexListVboCacheKey = indexListsVboCacheKeys.get(density);
		}
		//old:
        ...
        if (indexListVboId == null)
        {
            ...
            if (indexListVboCacheKey == null)
            {
				...
				//added support for GL_TRIANGLES_ADJACECY
				if(fillAdjacent){
                	indexListsVboAdjacencyCacheKeys.put(density, indexListVboCacheKey);
				}
				else{
    ...
    protected void renderTrianglesWithUniqueColors(DrawContext dc, RectTile tile)
    {
        ...
        try
        {
			...
			//Vito
			//added support for GL_TRIANGLES_ADJACECY
            if (dc.getGLRuntimeCapabilities().isUseVertexBufferObject() && this.bindVbos(dc, tile, 0, false))
			//old:
...
//        Vec4 tR = new Vec4(a[3], a[4], a[5]);
	...
	//X-START
	//Vito
	//create index buffer for rendering with GL_TRIANGLES_ADJACECY
	protected static void createIndicesAdjacency(int density)
    {
        if (density < 1)
            density = 1;

        if (indexListsAdjacency.containsKey(density))
            return;
        ...
        for (int i = 0; i < sideSize; i++)
        {
			for(int j = 0; j < sideSize; j++)
        	{
                int iminu1 = i-1 > -1 ? i-1 : i;
                int iplus1 = i+1 < sideSize ? i+1 : i;
                int iplus2 = i+2 < sideSize ? i+2 : (i+1 < sideSize ? i+1 : i);

                int jminu1 = j-1 > -1 ? j-1 : j;
                int jplus1 = j+1 < sideSize ? j+1 : j;
                int jplus2 = j+2 < sideSize ? j+2 : (j+1 < sideSize ? j+1 : j);
                
				int x = (i * (sideSize + 1)) + j;
				int xR = (i * (sideSize + 1)) + jplus1;
				int xL = ((iplus1) * (sideSize + 1)) + jminu1;
                int xD = ((iminu1) * (sideSize + 1)) + jplus1;
				int xU = ((iplus1) * (sideSize + 1)) + j;
				int xUU = ((iplus2) * (sideSize + 1)) + j;
				int xRR = (i * (sideSize + 1)) + jplus2;
				int xUR = (iplus1 * (sideSize + 1)) + jplus1;
        ...
        indexListsAdjacency.put(density, buffer);
    }
	//X-END
